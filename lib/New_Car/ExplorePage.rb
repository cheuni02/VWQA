class ExplorePage < NewCars
  ...
  def slides(type)
    ...
    when 'driving'
      index = 2
    else fail 'Not a valid type'
    end

    @browser.div(class: 'explore-holder', index: index).ul(class: 'indicator').lis
  end
  ...
  def current_slide(type)
    ...
    @browser.div(class: 'explore-holder', index: index).ul(class: 'indicator').li(class: 'current')
  end
end
