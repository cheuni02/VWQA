...
When(/^i land on the fleet page the slideshow should be visible$/) do
  fail unless @technologyLanding.fleet_carousel_present?
end
...
Then(/^i see the page opened having the following title (.*)$/) do |title|
  fail AssertionError, 'Incorrect page title' unless @technologyLanding.get_browser_title == title
end

When(/^i see the additional fleet sections on the page$/) do
  fail unless @technologyLanding.fleet_team_present?
  fail unless @technologyLanding.fleet_newsletter_present?
  fail unless @technologyLanding.fleet_manager_guides_preset?
  fail unless @technologyLanding.fleet_tax_guide_present?
  fail unless @technologyLanding.fleet_emissions_guide_present?
end
...
Then(/^i should be directed to fleet team page$/) do
  fail AssertionError, 'Incorrect page title' unless @technologyLanding.get_browser_title == 'Contact us : Volkswagen UK'
end
...
Then(/^i should be directed to a new tab with fleet managers page displayed$/) do
  fail AssertionError, 'Managers guide page is not opened' unless @technologyLanding.switch_to_manager_page
  fail AssertionError, 'Incorrect page title' unless @technologyLanding.get_browser_title == "Fleet manager's guide : Volkswagen UK"
end
