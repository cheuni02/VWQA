...
Then /^step 2 should be loaded successfully$/ do
  fail AssertionError, 'Step 2 not loaded' unless @booktestdrive.postcode_input_present?
end
...
Then /^a list of retailers should appear depending on if it's (.*)$/ do |result|
  if result == 'fail'
    fail AssertionError, 'Should show error text if invalid postcode' unless @booktestdrive.error_message_present?
  else
    fail AssertionError, 'Retailers not being shown' unless @booktestdrive.get_retailers.present?
  end
end
...
Then /^the 3rd step should be loaded$/ do
  fail AssertionError, '3rd step not loaded' unless @booktestdrive.step3_page_loaded?
end
