...
Then /^step 3 should be loaded$/ do
  fail AssertionError, 'Step 3 not loaded' unless @booktestdrive.step3_page_loaded?
end

And /^the number of days available should be "(.*)"$/ do |days|
  puts @booktestdrive.get_days.length
  fail AssertionError, "Incorrect number of days - expected #{days}" unless @booktestdrive.get_days.length == days.to_i
end
