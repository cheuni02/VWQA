...
Then /^validation should appear for mandatory fields$/ do
  raise ValidationError unless @booktestdrive.get_validation.length > 0
end
...
Then /^It should show confirmation of the test drive booking$/ do
  raise AssertionError, "Confirmation not loading" unless @booktestdrive.confirmation_page_loaded?
end
