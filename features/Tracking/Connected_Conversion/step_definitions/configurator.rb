...
Then /^I should see conversion\.js being referenced in a script tag$/ do
  raise AssertionError, "Tracking script not found = conversion.js" unless @tracking.find_conversion_script
end

And /^I should also see CDATA$/ do
  raise AssertionError, "Tracking script not found = CDATA" unless @tracking.conversion_cdata? && !@tracking.valid_id?
end

And /^I should also see a tracking pixel for nonJS users$/ do
  raise AssertionError, "Tracking script not correct - noscript" unless !@tracking.parse_con_noscript
end
...
And /^I should see an iframe generated via the tag$/ do
  raise AssertionError, "Iframe not found" unless @tracking.iframe?
end

And /^it should contain the correct information$/ do
  @attributes = @tracking.get_iframe_hash
  raise AssertionError, "Value of #{@attributes["U11"]} isn't false" unless @attributes["U11"] == "false"
end
...
Then /^the finished attribute, known as U11, should read "([^"]*)"$/ do |value|
  @attributes = @tracking.get_iframe_hash
  raise AssertionError, "Value of #{@attributes["U11"]} isn't true" if @attributes["U11"] != value
end
...
Then /^the iframe should contain the correct information$/ do
  ...
  @attributes.each do |key, value|
    raise AssertionError, "Value of #{key} is empty" unless value != "" && !value.nil?
  end

  raise AssertionError, "Value of U1 (model) isn't correct" unless @attributes["U1"] == @data[:model]
  raise AssertionError, "Value of U2 (trim) isn't correct " unless @attributes["U2"] == @data[:trim]
  raise AssertionError, "Value of U3 (derivative) isn't correct" unless @attributes["U3"] == @data[:derivative]
end
...
Then /^the iframe contents should change accordingly$/ do
  ...
  unchanged = ["U1", "U2", "U8", "U9"] # Fields that aren't going to change.
  @attributes.each do |key, value|
    raise AssertionError, "Value of #{key} hasn't changed" unless value != attributesNew[key] || unchanged.include?(key)
  end
end
