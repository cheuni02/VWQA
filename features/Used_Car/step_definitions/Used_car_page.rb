...
Then /^I should find that the images have loaded properly$/ do
  fail ElementNotPresentError unless @thumbnails_present
end
...
Then /^i should see that the youtube videos are present on the page$/ do
  @youtube_used_videos.each do |_key, _video|
    fail ElementNotPresentError, "Video #{@youtube_used_videos} not found!" unless @youtube_used_videos
  end
end
...
Then /^the benefits sections should have loaded on the page sucessfully$/ do
  ...
  fail ElementNotPresentError unless @used_cars_page.benefit_sections
  @all_links_in_section.each do |item|
    fail ElementNotPresentError, "#{item} not present" unless item.present?
  end
end
...
Then /^the (.*) image should load$/ do |benefit|
  ...
  fail ElementNotPresentError unless @images_for_benefits_present
end
...
Then /^i should see that the link is successfully on the page$/ do
  fail ElementNotPresentError unless @used_cars_button
end
