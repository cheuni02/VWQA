...
Then /^i should see the buying guide page load in my browser$/ do
  raise ValidationException, "Error" unless site.finance_offers.offers_page_loaded?  
end
...
Then /^i should see the Volkswagen offers page load in my browser$/ do
  raise unless site.finance_offers.buying_guide.page_loaded?
end
...
Then /^i should see a information page with a header "(.*)"$/ do |header|
  raise unless site.standard_content_page.get_header_text =~ /#{header}/i
end  
  
And /^there should be an image and text information on the page$/ do
  raise unless site.standard_content_page.image_loaded?  
end
...
And /^i should see (\d+) other buttons pointing to other pages on the site$/ do |count|
  raise unless site.standard_content_page.count_button_links == count.to_i  
end
...
And /^there should be an image and (\d+) other button link displayed on the page$/ do |links|
  raise unless site.standard_content_page.image_loaded? and site.standard_content_page.count_button_links == links.to_i 
end
