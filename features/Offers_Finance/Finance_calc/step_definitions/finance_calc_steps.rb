...
Then /^user should be navigated to the models finance calculator page$/ do
  ...
  raise PageNotLoadedError unless @finance_calc.finance_table_present?
end
...
Then /^trims finance page should show (.*) derivative above table$/ do |trim_name|
  ...
  raise AssertionError, "#{trim_name}:not seen above finance table" unless check.match(/#{trim_name}/) != nil
end
...
Then /^the deposit value is reflected on the table is "(.*)"$/ do |depositValueCheck|
  ...
  raise AssertionError, " #{depositValueCheck}: not seen on finance table" unless check == depositValueCheck
end
...
Then /^the duration field within the table should show month (.*)$/ do |month|
  ...
  raise AssertionError, "#{month}: selected but not seen on the table" unless durationMatch != nil
end
...
Then /^the average miles per year field within the table should show mileage (.*)$/ do |mileage|
  ...
  raise AssertionError, "#{mileage}: selected but not seen on the table" unless mileage == copyToCheckMod
end
