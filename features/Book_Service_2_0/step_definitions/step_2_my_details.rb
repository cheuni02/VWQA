...
Then(/^I will see my personal details in summary:$/) do |table|
  ...
  table.hashes.each do |hash|
    expect(service_booking.summary_title.text).to eq(hash['Title'])
    expect(service_booking.summary_name.text).to eq(hash['Name'])
    expect(service_booking.summary_surname.text).to eq(hash['Surname'])
    expect(service_booking.summary_mobile.text).to eq(hash['Mobile'])
    expect(service_booking.summary_email.text).to eq(hash['Email'])
  end
end
...
Then(/^I will see my address details in summary:$/) do |table|
  service_booking = site.service_booking.step3
  table.hashes.each do |hash|
    expect(service_booking.summary_house_number.text).to eq(hash['House No'])
    expect(service_booking.summary_street.text).to eq(hash['Street'])
    expect(service_booking.summary_town.text).to eq(hash['City'])
    expect(service_booking.summary_postcode.text).to eq(hash['Postcode'])
  end
end
