Given /^i visit the Volkswagen Homepage$/ do
  ...
  site.set_cookies('pa', ENV['PCOOKIE'], path: '/ProvIM/ProvAnalytics/', expires: Time.now + 86_400, secure: false)
...
And /^i ensure that i have been selected to use the randomised personalisation flow$/ do
  ...
  site.set_cookies('ico-cookies', 'true', path: '/', expires: Time.now + 86_400, secure: false)
...
Then /^i should see the standard set of links present on the left side of the main page$/ do
  ...
  fail ElementNotPresentError, "Could not find homepage text element #{elem1}" unless site.homepage.quick_link_present?(elem1) == 1
  fail ElementNotPresentError, "Could not find homepage text element #{elem2}" unless site.homepage.quick_link_present?(elem2) == 1
end
...
Then /^i should see homepage links to "(.*)" and "(.*)"$/ do |link1, link2|
  ...
  fail ElementNotPresentError, "Could not find homepage quick link element #{link1}" unless site.homepage.quick_link_present?(link1) == 1
  fail ElementNotPresentError, "Could not find homepage quick link element #{link2}" unless site.homepage.quick_link_present?(link2) == 1
end
...
Then /^i should see the (.*) image appear in my homepage carousel$/ do |image_name|
  ...
  site.refresh_browser
  sleep(5)
  fail ElementNotPresentError, "Could not find image with title #{image_name}" unless site.homepage.find_homepage_image(image_name)
end
...
And /^i click on my personalised link to "(.*)"$/ do |link_text|
  fail ElementNotPresentError, "Could not find homepage quick link element #{link_text}" unless site.homepage.quick_link_present?(link_text)
  site.homepage.click_quick_link(link_text)
end

Then /^i should be taken to the book a test drive page for the "(.*)" Model$/ do |model|
  fail PageNotLoadedError, "Book Test drive page for model #{model} was not loaded!" unless site.book_test_drive.postcode_input_present?
end
...
Then /^i should see the configure page in my browser load for the model i selected$/ do
  fail PageNotLoadedError, "Car Configurator Selection page not loaded for the #{@model} Model" unless site.new_cars.configure_page.page_loaded?
end
