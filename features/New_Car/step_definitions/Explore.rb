...
Then /^explore page should load correctly$/ do
  raise AssertionError, "Unable to load page" unless @explore.page_loaded?
end
...
Then /^the image should change into the 360 degree model$/ do
  raise AssertionError, "360 view not loading correctly" unless @explore.get_360_images.length > 0
  raise AssertionError, "360 view not loading correctly" unless @explore.get_360_arrow.present?
end
...
Then /^it should take me to slide "(.*)"$/ do |slide|
  raise AssertionError, "not navigating to interior gallery" unless @explore.get_current_slide("interior").text == slide
end

Then /^there should be "(.*)" slides$/ do |slides|
  raise AssertionError, "not showing slides correctly" unless @explore.get_slides("interior").length == slides.to_i
end
...
Then /^I'll load slide "(.*)"$/ do |slide|
  raise AssertionError, "not navigating to stylish gallery" unless @explore.get_current_slide("stylish").text == slide
end

Then /^there'll be "(.*)" slides$/ do |slides|
  raise AssertionError, "not showing slides correctly" unless @explore.get_slides("stylish").length == slides.to_i
end
...
Then /^I'll load slide "(.*)" which is the last$/ do |slide|
  raise AssertionError, "not navigating to driving gallery" unless @explore.get_current_slide("driving").text == slide
end

Then /^there will be "(.*)" slides$/ do |slides|
  raise AssertionError, "not showing slides correctly" unless @explore.get_slides("driving").length == slides.to_i
end
