...
Then /^i should see an error message shown in my browser telling me to select a time$/ do
  ...
  raise AssertionError, "Error messages displayed in form entry, expected no errors!" unless errors.length > 0
end
...
Then /^i should see two further insurance based options appear on my screen$/ do
  raise ElementNotPresentError, "Could not find loan car (self insured) convience option element!" unless @serviceStep4.loan_car_self_insured_option_present?
  raise ElementNotPresentError, "Could not find loan car (added insurance) convience option element!" unless @serviceStep4.loan_car_with_insurance_option_present?
end
...
Then /^i should see another two options relating to the distance required in order to have my car collected$/ do
  raise ElementNotPresentError, "Could not find car collection (Up to 20 mins) convience option element!" unless @serviceStep4.up_to_twenty_mins_present?
  raise ElementNotPresentError, "Could not find car collection (Up to 40 mins) convience option element!" unless @serviceStep4.up_to_fourty_mins_present?
end
...
Then /^i should see a form requiring me to enter my personal details$/ do
  raise PageNotLoadedError, "Step 5 of book a service was not loaded!" unless site.book_service.booking_step5.page_loaded?
end
