...
When(/^i land on the technology page$/) do
  fail unless @technologyLanding.page_loaded?
end
...
Then(/^i should be directed to a new tab with technology managers page displayed$/) do
  ...
  fail AssertionError, 'Incorrect page title' unless @technologyLanding.get_browser_title == "technology manager's guide : Volkswagen UK"
end
